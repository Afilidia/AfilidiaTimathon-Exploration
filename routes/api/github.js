...
router.get('/commit', function(req, res, next) {
    if(req.header['x-hub-signature'] != "sha1=" + crypto.createHmac('sha1', process.env.GITHUBSECRET||"T9QJVrfatKB9uYKxe4dKtWiiq2vAUpxUZSmTYPeG").update(chunk.toString()).digest('hex')) return res.render('index', {});
    res.render('index', {});
    process.exit(0);
